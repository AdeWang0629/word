...
class Account_settings extends CI_Controller {
	...
	function __construct()
	{
		...
		$this->load->model(array('account/account_model', 'account/account_details_model', 'account/ref_country_model', 'account/ref_language_model', 'account/ref_zoneinfo_model', 'account/ref_currency_model'));
	...
	function index()
	{
		...
		if ($this->form_validation->run())
		{
			...
			else
			{
				...
				///added by sumon on 09-02-17
				$attributes['skype_id'] = $this->input->post('settings_skype_id', TRUE) ? $this->input->post('settings_skype_id', TRUE) : NULL;
				...
				$attributes['currency'] = $this->input->post('settings_currency', TRUE) ? $this->input->post('settings_currency', TRUE) : NULL;
