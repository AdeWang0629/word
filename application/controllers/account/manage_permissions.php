...
class Manage_permissions extends CI_Controller {
  ...
  function index()
  {
    ...
    $data['title'] = lang('permissions_page_name');
	...
	$config = array();
	$config['base_url'] = base_url().'account/manage_permissions/index/';
	
	$config['total_rows'] = $this->acl_permission_model->no_of_rows();
	
	$config['num_links'] = 3;
	$config['per_page'] = $this->config->item("pagination_perpage");
	$config['uri_segment'] = 4;
	
	$config['full_tag_open'] = '<nav><ul class="pagination pagination-sm" style="margin:0;">';
    ...
    $permissions = $this->acl_permission_model->get($page,$config['per_page']);
	...
	/*echo "<pre>"; print_r($data['permissions']); echo "<pre>"; 
	exit;*/
