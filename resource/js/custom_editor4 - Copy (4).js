...
//    max_width: 500,
    ...
    setup: function (editor) {
        ...
        editor.on('paste', function (event) {
            ...
            //         var pp = strSplitOnLength(str, 542);
            // }, 100);
            var paginatedText = tinymce.activeEditor.dom.select('div#page_separation');
            ...
            if (paginatedText != '') {
                ...
                setTimeout(function () {
                    ...
                    var str = tinymce.activeEditor.getContent({format: 'raw'});
                    var text_array = tinymce.activeEditor.dom.select('p')[1];
                    var text_value = text_array.innerHTML;
                     var text_value1 = '<p>'+text_value+'</p>';

                    rearrange(text_value1, 542, 2, text_value);
                ...
                setTimeout(function () {
                    var str = tinymce.activeEditor.getContent({format: 'raw'});
                    // alert(str);
                    // var final_text='<p style="display:none;">'+str+'</p>';
                    // tinymce.execCommand('mceInsertContent', false, final_text);
                    rearrange(str, 542, 1);
                }, 100);
            }
            // save_autometically();
        ...
        editor.on('change', function (event) {
            ...
            if (span_tag_arr != '') {
                ...
                if (get_font_size == '48px') {
                    ...
                    var text_value = text_array.innerHTML;
                    var text_value1 = '<p>' + text_value + '</p>';
                    rearrange(text_value1, 1850, 2, text_value);
                }
            }
...
function rearrange(text, length, id, text_value_2) {
    if (id == 1) {
        var text_array = tinymce.activeEditor.dom.select('p')[0];
        var text_value = text_array.innerHTML;

    } else {
        var text_value = text_value_2;

        // alert(text);
    }

    // var chunks = splitText(text, false, length);
    // var chunks = strSplitOnLength(text,600);
    // console.log(chunks);
    var value = '';
    var value1 = '';
    ...
    for (var i = 0; i < textArray.length; i++) {
        ...
        // tinymce.activeEditor.insertContent('<!-- pagebreak -->');
        value1 += '<div style="display: block;height: 842px; width:700px;margin-left:10px;">' + text[i] + '</div><!-- pagebreak -->';
        // value += '<div id="paginate"><p>' + chunks[i] + '</p></div>';
        // value += '<p>' + chunks[i] + '</p><!-- pagebreak -->';
    }
    var value = '<div id="page_separation">' + value1 + '</div>';
    // tinymce.activeEditor.setContent('');
    tinymce.activeEditor.setContent('<p style="display: none">' + text_value + '</p>' + value);

}

function strSplitOnLength(string, width) {
    arr = [];
...
function createPage() {
    ...
    // p.append(node); // appends the element to the container for all the pages
...
function appendToLastPage(word) {
    ...
    var pageText = page.innerHTML.replace('style="display: none;" data-mce-style="display: none;"&gt;', ''); // gets the text from the last page
